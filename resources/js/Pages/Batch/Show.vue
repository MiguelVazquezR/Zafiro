<template>
    <Head>
        <title>{{ batch.data.name }} | Ingeniería Zafiro</title>
    ...
    <div class="relative bg-slate-50 min-h-screen font-sans selection:bg-primary selection:text-black">
        ...
        <transition name="fade">
            <div v-if="showPreview" class="fixed inset-0 z-[60] flex justify-center items-center">
                ...
                <button v-if="batch.data.images?.length > 1" @click.stop="prevImage" class="absolute left-4 sm:left-10 top-1/2 -translate-y-1/2 w-14 h-14 bg-white/10 hover:bg-white/20 text-white rounded-full flex items-center justify-center backdrop-blur-sm transition-all z-20">
                    <i class="fa-solid fa-chevron-left text-xl"></i>
                </button>
                
                <button v-if="batch.data.images?.length > 1" @click.stop="nextImage" class="absolute right-4 sm:right-10 top-1/2 -translate-y-1/2 w-14 h-14 bg-white/10 hover:bg-white/20 text-white rounded-full flex items-center justify-center backdrop-blur-sm transition-all z-20">
                ...
                <div class="relative z-10 w-full max-w-5xl px-4 flex flex-col items-center">
                    <img class="w-full max-h-[80vh] object-contain rounded-xl shadow-2xl" 
                         :src="batch.data.images[currentImage]?.original_url" 
                         alt="Vista del Lote">
                    <p class="text-white mt-4 font-medium tracking-widest text-sm">
                        {{ currentImage + 1 }} / {{ batch.data.images?.length || 1 }}
                    </p>
                </div>
            </div>
        </transition>
        ...
        <!-- NAVEGACIÓN SUPERIOR (Unificada con Lotes.vue) -->
        ...
        <main class="pt-24 lg:pt-32 pb-16 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex flex-wrap items-center gap-2 text-sm text-slate-500 font-medium mb-8 bg-white px-5 py-3 rounded-full w-max shadow-sm border border-slate-100">
                ...
                <Link v-if="batch.data.subdivision" :href="route('subdivisions.show', batch.data.subdivision?.id)" class="hover:text-primary transition-colors">
                    {{ batch.data.subdivision?.name }}
                </Link>
                <i v-if="batch.data.subdivision" class="fa-solid fa-chevron-right text-[10px] text-slate-300"></i>
                <span class="text-slate-800 font-bold">{{ batch.data.name }}</span>
            </div>
            ...
            <div class="mb-8">
                <div class="flex items-start justify-between gap-4">
                    <div>
                        <span class="px-3 py-1 bg-primary/20 text-primary border border-primary/30 rounded-full text-[10px] font-black tracking-widest uppercase mb-3 inline-block">
                            Terreno en Venta
                        </span>
                        <h1 class="text-3xl lg:text-5xl font-black text-slate-900 tracking-tight">{{ batch.data.name }}</h1>
                        <p class="text-lg text-slate-500 mt-3 flex items-center gap-2">
                            <i class="fa-solid fa-location-dot text-primary"></i> {{ batch.data.address }}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="relative grid grid-cols-1 lg:grid-cols-4 gap-3 lg:gap-4 h-[400px] lg:h-[500px] mb-12 rounded-[32px] overflow-hidden group">
                ...
                <div class="lg:col-span-2 h-full cursor-pointer overflow-hidden" @click="openGallery(0)">
                    <img class="w-full h-full object-cover hover:scale-105 transition-transform duration-500" 
                         :src="batch.data.images[0]?.original_url" alt="Imagen principal">
                </div>
                ...
                <div class="hidden lg:grid grid-cols-2 grid-rows-2 col-span-2 gap-4 h-full">
                    <div v-for="i in 4" :key="i" class="h-full w-full relative overflow-hidden cursor-pointer" @click="openGallery(i)">
                        <img v-if="batch.data.images[i]" 
                             class="w-full h-full object-cover hover:scale-105 transition-transform duration-500" 
                             :src="batch.data.images[i]?.original_url" alt="Imagen secundaria">
                        <div v-else class="w-full h-full bg-slate-200"></div>
                        
                        <!-- Overlay "Ver todas" en la última imagen -->
                        <div v-if="i === 4 && batch.data.images?.length > 5" 
                ...
                <button @click="openGallery(0)" class="absolute bottom-6 right-6 bg-white/90 backdrop-blur-md text-slate-900 px-6 py-3 rounded-full font-bold shadow-lg hover:bg-white transition-all flex items-center gap-2">
                    <i class="fa-regular fa-images"></i> Mostrar {{ batch.data.images?.length || 0 }} fotos
                </button>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-12 items-start">
                ...
                <!-- Columna Izquierda (Características y Mapa) -->
                <div class="lg:col-span-2 space-y-12">
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
                        <div class="bg-white p-5 rounded-[24px] border border-slate-100 shadow-sm flex flex-col items-center justify-center text-center">
                            <i class="fa-solid fa-ruler-combined text-2xl text-primary mb-3"></i>
                            <p class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Superficie</p>
                            <p class="text-lg font-black text-slate-800">{{ batch.data.surface }} m²</p>
                        </div>
                        <div class="bg-white p-5 rounded-[24px] border border-slate-100 shadow-sm flex flex-col items-center justify-center text-center">
                            <i class="fa-solid fa-money-bill-wave text-2xl text-primary mb-3"></i>
                            <p class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Precio</p>
                            <p class="text-lg font-black text-slate-800">${{ formatCurrency(batch.data.price) }}</p>
                        </div>
                        <div class="bg-white p-5 rounded-[24px] border border-slate-100 shadow-sm flex flex-col items-center justify-center text-center">
                            <i class="fa-solid fa-map-location-dot text-2xl text-primary mb-3"></i>
                            <p class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Ubicación</p>
                            <p class="text-[13px] font-bold text-slate-800 leading-tight truncate w-full" :title="batch.data.address">{{ batch.data.address }}</p>
                        </div>
                        <!-- Planos (Si existen) -->
                        <a v-if="batch.data.planos?.length > 0" :href="batch.data.planos[0]?.original_url" target="_blank"
                    ...
                    <div class="bg-white p-8 sm:p-10 rounded-[32px] shadow-sm border border-slate-100">
                        <h2 class="text-2xl font-black text-slate-900 mb-6">Acerca del Lote</h2>
                        <p class="text-slate-600 leading-relaxed text-lg whitespace-pre-line">{{ batch.data.description }}</p>
                    </div>

                    <!-- Ubicación en Mapa -->
                    <div v-if="batch.data.lon && batch.data.lat" class="bg-white p-8 sm:p-10 rounded-[32px] shadow-sm border border-slate-100">
                ...
                <div class="lg:col-span-1">
                    <div class="sticky top-32 bg-white rounded-[32px] shadow-xl shadow-slate-200/50 border border-slate-100 p-8">
                        <p class="text-xs font-black text-slate-400 uppercase tracking-widest mb-2">Inversión Estimada</p>
                        <p class="text-4xl font-black text-slate-900 mb-6">${{ formatCurrency(batch.data.price) }} <span class="text-lg font-bold text-slate-400">MXN</span></p>
                        
                        <div class="space-y-4 mb-8">
                            <div class="flex justify-between items-center py-3 border-b border-slate-100">
                                <span class="text-slate-500 font-medium">Fraccionamiento</span>
                                <span class="text-slate-900 font-bold text-right">{{ batch.data.subdivision?.name || 'Independiente' }}</span>
                            </div>
                            <div v-if="batch.data.subdivision?.type" class="flex justify-between items-center py-3 border-b border-slate-100">
                                <span class="text-slate-500 font-medium">Tipo de Lote</span>
                                <span class="text-slate-900 font-bold text-right">{{ batch.data.subdivision?.type }}</span>
                            </div>
                            <div class="flex justify-between items-center py-3 border-b border-slate-100">
                                <span class="text-slate-500 font-medium">Dimensiones</span>
                                <span class="text-slate-900 font-bold">{{ batch.data.surface }} m²</span>
                            </div>
                        </div>
                        ...
                        <p class="text-center text-xs text-slate-400 font-medium mt-4">
                            Sin compromiso. Resolvemos tus dudas de inmediato.
                        </p>
                    </div>
                </div>
        ...
        <!-- FOOTER PREMIUM (Unificado con Lotes.vue) -->
...
// Lógica pre-armada de enlace WhatsApp para evitar cadenas gigantes en el template
const whatsappLink = computed(() => {
    const text = encodeURIComponent(`Hola! vi la página, me interesa obtener información sobre el terreno: ${props.batch.data.name}`);
    return `https://api.whatsapp.com/send?phone=523329281702&text=${text}`;
});

// DATOS ESTÁTICOS PARA FOOTER (Idénticos a Lotes.vue)
const lotes = [
...
const openGallery = (index) => {
    if (props.batch.data.images && props.batch.data.images[index]) {
...
const nextImage = () => {
    const total = props.batch.data.images?.length || 1;
...
const initMap = () => {
    if (window.google && props.batch.data.lat && props.batch.data.lon) {
        const coords = { 
            lat: parseFloat(props.batch.data.lat), 
            lng: parseFloat(props.batch.data.lon) 
        };
...
onMounted(() => {
    ...
    if (mapContainer.value) {
        // En caso de que la API de Google Maps ya esté lista, inicializamos el mapa
        initMap();
    }
});
