        ...
        <div class="w-5/6 lg:w-[80%] mx-auto rounded-md lg:border border-secondary p-5">
            ...
            <div class="lg:grid grid-cols-3 gap-x-3 mt-3">
                <div>
                    ...
                    <InputFilePreview v-show="currentImage == 2" :imageUrl="batch.media[1]?.original_url" @cleared="form.cleared_cover2 = true" @imagen="this.form.image_cover2 = $event;" />
                    <InputFilePreview v-show="currentImage == 3" :imageUrl="batch.media[2]?.original_url" @cleared="form.cleared_cover3 = true" @imagen="this.form.image_cover3 = $event;" />
                    <InputFilePreview v-show="currentImage == 4" :imageUrl="batch.media[3]?.original_url" @cleared="form.cleared_cover4 = true" @imagen="this.form.image_cover4 = $event;" />
                    <InputFilePreview v-show="currentImage == 5" :imageUrl="batch.media[4]?.original_url" @cleared="form.cleared_cover5 = true" @imagen="this.form.image_cover5 = $event;" />
                    <InputFilePreview v-show="currentImage == 6" :imageUrl="batch.media[5]?.original_url" @cleared="form.cleared_cover6 = true" @imagen="this.form.image_cover6 = $event;" />
                    <InputFilePreview v-show="currentImage == 7" :imageUrl="batch.media[6]?.original_url" @cleared="form.cleared_cover7 = true" @imagen="this.form.image_cover7 = $event;" />
                    <InputFilePreview v-show="currentImage == 8" :imageUrl="batch.media[7]?.original_url" @cleared="form.cleared_cover8 = true" @imagen="this.form.image_cover8 = $event;" />
...
data() {
    const form = useForm({
        ...
        subdivision_id: this.batch.subdivision_id,
...
methods:{
   update() {
        if (this.form.planos != null || this.form.image_cover1 != null) {
            ...
            this.form.post(route("batches.update-with-media", this.batch.id), {
                onSuccess: () => {
                    this.$notify({
                        title: "Correcto",
                        message: "Se ha editado el lote",
                        type: "success",
                    });
                },
            });
        } else {
            this.form.put(route("batches.update", this.batch.id), {
                onSuccess: () => {
                    this.$notify({
                        title: "Correcto",
                        message: "Se ha editado el lote",
