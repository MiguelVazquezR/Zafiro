    ...
    <div class="bg-white rounded-xl border shadow-md p-3 w-[410px] md:w-[550px] h-[240px] flex space-x-4 cursor-pointer">
        <figure class="bg-gray-300 rounded-[20px] h-full w-3/4 relative">
            ...
            <div v-if="$page.props.auth?.user?.id" class="flex items-center space-x-2 absolute top-2 right-2">
                <i @click.stop="$inertia.get(route('batches.edit', batch.id))" class="fa-solid fa-pencil text-white text-sm pl-[9px] pt-[6px] w-8 h-8 rounded-full bg-black/60 hover:scale-105"></i>
